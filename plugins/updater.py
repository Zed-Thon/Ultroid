...
UPSTREAM_REPO_URL = "https://github.com/Zed-Thon/Ultroid"
...
async def upstream(ups):
    ...
    if "الان" not in conf:
        if changelog:
            ...
            tldr_str = tl_chnglog + f"\n\n- استخـدم {hndlr}تحديث الان لتحديـث البــوت"
    ...
    if Var.HEROKU_API is not None:
        ...
        try:
            ...
            repo.__del__()
            return
        await eod(pagal, "**- تم التحديث .. بنجـاح ✓**\n**- انتظـر بضـع دقائـق ... جـاري اعـادة تشغيـل البـوت**", time=60)
    else:
        ...
        await eod(
            pagal,
            "**- تم التحديث .. بنجـاح ✓**\n**- انتظـر بضـع دقائـق ... جـاري اعـادة تشغيـل البـوت**",
        )
